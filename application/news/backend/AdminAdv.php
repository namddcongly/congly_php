...
class AdminAdv extends Form
{
	...
	function admin_adv(){
       ...
       foreach($list_adv as $row)
       {
            ...
            if($row['video'])
                $video = IMG::show('adv/'.date('Y/n',$row['time_created']).'/',$row['video']); 
                   
            $file_type =  substr(strrchr($row['img'],'.'),1);
            if($file_type && in_array($file_type,$array_ext)&& $row['video'] == '')
                joc()->set_var('banner','<img width="120px;" src="'.$img.'" />');
            elseif($file_type == 'swf' && $row['video'] == '')
                joc()->set_var('banner','<p style="height: 260px"><embed align="middle" width="300" height="250" src="'.$img.'" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" allowscriptaccess="always" wmode="transparent" quality="high"></p>');   
            else
                joc()->set_var('banner','<object id="FlashID" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="270" height="205">
            							<param name="movie" value="http://congly.duluan.com.vn/data/video/mediaplayer.swf" />
            							<param name="quality" value="high" />
            							<param name="wmode" value="opaque" />
            							<param name="swfversion" value="6.0.65.0" />
            							<param name="flashvars" value="width=270&height=205&file=http://congly.duluan.com.vn/'.$video.'&image=http://congly.duluan.com.vn/'.$img.'" />
            							<param name="expressinstall" value="scripts/expressInstall.swf" />
            							<!--[if !IE]>-->
            							<object type="application/x-shockwave-flash" data="http://congly.duluan.com.vn/data/video/mediaplayer.swf" width="270" height="205">
            								<!--<![endif]-->
            								<param name="quality" value="high" />
            								<param name="wmode" value="opaque" />
            								<param name="swfversion" value="6.0.65.0" />
