...
class FrontendNews
{
    ...
    public function readSeo($id)
    {
        ...
        if (!$id)
            return array();
        return self::$dbNews->selectOne('seo', '*', 'nw_id=' . $id);
    }

    /**
     * Lấy danh sách tin trên trang chủ
     */
    public function getNewsHome($list_field, $wh = null, $limit = null, $is_cache = false)
    {
        if ($is_cache) {
            $Cache = new CacheFile();
            $data = $Cache->get(md5('store_home' . $wh), '', CACHE_FILE_PATH . 'news' . DS, 300);
            if ($data) {
                return $data;
            }
        }

        $result = self::$dbNews->select('store_home', $list_field, $wh, 'arrange ASC, time_public DESC', $limit, 'nw_id');
        if ($is_cache) {
            $Cache->set(md5('store_home' . $wh), $result, 300, '', CACHE_FILE_PATH . 'news' . DS);
        }
        return $result;
    }
    ...
    function getNews($table, $list_field, $wh = null, $order = null, $limit = null, $key = null, $is_cache = false)
    {
        ...
        if ($table == 'store') {
            if ($wh == null)
                ...
                $wh .= " AND time_public > 0 AND time_public < " . time();
        }
    ...
    {
        $list_field = 'id,cate_id,cate_path,type_post,title,description,tag,time_created,time_public,author,img1,img2,province_id,img3,img4,file,origin,topic_id,relate,is_video,is_img,poll_id';
    ...
    function newsOther($cond = "", $limit = '0,10')
    {
        ...
        return self::$dbNews->select("store", "id AS nw_id,title,cate_id,time_created,description,time_public,img2,img1", $cond, "id DESC", $limit);
    }
    ...
    function getIdNewsMoreComments()
    {
        $sql = 'SELECT nw_id,count(nw_id) AS total FROM comment WHERE status = 1 AND time_post >  ' . (time() - 7 * 86400) . ' GROUP BY nw_id ORDER BY  count(nw_id) DESC LIMIT 0,5';
        self::$dbNews->query($sql);
        $data = self::$dbNews->fetchAll('nw_id');
        return $data;
    }
