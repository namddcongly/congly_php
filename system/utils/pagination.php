...
{	
	...
	function create1_rewrite($ext)
	{
	    ...
	    if($total_page > 1)
	    {
	        ...
	        for($i = $start; $i < $end; ++$i)
	        {
	            $paging .= $this->li_open.(($this->current_li && $this->page == $i) ? ' style="color:red" class="'.$this->class_current.'">' : '>');
	            
	            $link = $ext."/trang-".($i+1)."/";
	            
	            $paging .= $this->a_open.'"'.$link.'" '.((!$this->current_li && $this->page == $i) ? ' style="color:red" class="'.$this->class_current.'">' : '>').($i+1).$this->a_close;
